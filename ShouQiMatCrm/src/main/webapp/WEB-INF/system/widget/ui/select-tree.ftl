...
<#if list?is_sequence>
	<#if listKey!="" && listValue!="">
		<#if listDeep!="" && list?size gt 0>
		<#local origDeep=list[0][listDeep]+1/></#if>
		<#list list as item>
			<option value="${item[listKey]}"<#if item[listKey]?string==value?string> selected="selected"</#if>><#if listDeep!="" && item[listDeep] gte origDeep><#list origDeep..item[listDeep] as i>&nbsp;&nbsp;</#list>></#if>${item.fcChannelExt[listValue]!}</option><#t/>
		</#list>
	<#else>
		<#list list as item>
